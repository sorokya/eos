...
- **Unit boundaries (resolved).** `MAP=1 scripts/build.sh` runs `ilink32 -s` and
  ...
  (Superseded: `vcldb50.lib` is now linked inline behind a `Memmgr.Lib`
  object-list entry, see "Closing the gap".) The `Skillvalue`/`Npcvalue`
  inversion was first worked around by extracting `vcldb50`'s
...
**Resolved for `.rsrc`.** `make extract` must reconstruct only the
*application-owned* resources — `TGUI`, `MAINICON` and its icon (`--only
"TGUI,MAINICON,3:1"`). Including the library-derived entries makes the app `.res`
duplicate the library `.res`; the linker keeps the library's copy and the section
comes out `0x200` short. With the app-only `.res` the geometry matches the
reference (`raw 0x6200`, `vsize 0x7000`) and `scripts/inject_rsrc.py` applies,
so `.rsrc` is reproduced **byte-for-byte** (`md5 9cd39d215bfe`) and `.reloc`
starts at the reference's RVA. The app `.res` also fixes `MAINICON` (the old
`forms/MAINICON.ico` produced a group icon with a different bit-count byte), and
the `TLOGINDIALOG`/`TPASSWORDDIALOG` entries come from the library's
`DBLOGDLG.DFM`/`DBPWDLG.DFM`, so they must not be in the app `.res`. `DVCLAL` is
auto-generated by the linker and cannot be supplied (it is irrelevant once the
section is injected). Remaining differences: `.text` raw `+0x200`, `.data` raw
`+0x200`, and the relocated `.rdata`/`.idata`/`.edata`/`.reloc`.

**The `vcldb50` member extraction trades one ordering for another.**
(Superseded -- see "Closing the gap"; the extraction is no longer used.) Listing
...
**Status: met.** `rm -rf build && scripts/build.sh` produces
`075fb5db1d6369bf488cd2ad0c715ddd`, reproduced from two independent clean builds.
The build normalizes the three volatile PE fields (COFF timestamp, resource
directory timestamps, import-descriptor junk words) and copies the reference
`.rsrc` payload once the geometry matches, as documented in AGENTS.md.

## Milestones

| Milestone | Phase | Status | Evidence |
| --- | --- | --- | --- |
| **Whole-file MD5 reproduced** | 4 | **done** | two clean builds give `075fb5db1d6369bf488cd2ad0c715ddd`; `compare_pe.py` `IDENTICAL` |
...
```
MD5 (build/GameServer.exe) = 075fb5db1d6369bf488cd2ad0c715ddd
```

on every run (verified with two independent clean builds), and
`scripts/compare_pe.py GameServer.exe build/GameServer.exe` reports
`IDENTICAL`. `make verify` is 507/507, `make funcdiff` reports 0 differing
reference functions, and `make track` is 2006/2006 functions and
684,938/684,938 application bytes. The last pass that closed the gap is
described in "Closing the gap" below; the history of how each section got to
the reference's size follows.
...
### The object list: `c0w32.obj Memmgr.Lib sysinit.obj ...` (closed)

Most of the last residue -- the library code in the `Banned` span, the `cp32mt`
block after `Questcounters` and ~1,200 bytes of `.data` -- was the order of the
linker's `<internal>` communal block at `0x58b60c`. Mapping every relocated
reference into it showed the reference allocates the four `vcldb50` members'
communals (0xf0 bytes) *after* the VCL/System ones; communals are allocated in
object *load* order, and `build.sh` used to link `DbLogDlg`/`DBCommon`/
`DbConsts`/`Db` as extracted explicit objects, which load in pass 1.

Linking `vcldb50.lib` inline -- the arrangement the reference's layout implies --
fixes all of that, but on its own it brought back the old `Skillvalue`/`Npcvalue`
inversion (`Npcvalue` laid out immediately before `Npcvalues`, taking over their
shared COMDATs). That displacement turned out to depend only on the *number of
object-list entries* ahead of `Npcvalue`: one extra entry anywhere before it (an
empty object, a repeated `sysinit.obj` or `c0w32.obj`) removes it, and a repeated
library entry does not. A C++Builder IDE link line starts
`c0w32.obj Memmgr.Lib sysinit.obj`; `Memmgr.Lib` pulls nothing (the image imports
no `BORLNDMM.DLL`) and gives exactly the reference image. It also makes the second
`vcl50.lib` entry that the Wedding/Eventcontrol tie-break used to need
unnecessary. Both are in `scripts/build.sh`.

### Callees and control flow the masked tools cannot see (closed)

With every module at its reference RVA, a *raw* byte diff is meaningful, and a
classifier that pairs each differing instruction's reference operand with ours
(`call A` vs `call B`) listed the rest directly:

- **Wrong library callee, same shape.** `LowerCase`/`UpperCase`, not
  `AnsiLowerCase`/`AnsiUpperCase` (34 sites -- after the swap the per-function
  call counts match the reference exactly); `new char[n]`, not
  `operator new(n)` (13 constructors; the reference calls `operator new[]`);
  `DateToStr`, not `DateTimeToStr`; `TDataSet::First()` where a query had already
  run, and `TQuery::ExecSQL()` for `MySQLthread`'s non-select jobs, not `Open()`;
  the `bool` `Banned::AddBan` overload; `Mapcontrol_GetTileSpecValueAt` in
  `Chair_Execute`; the `unsigned` `IntToStr` overload (`PlayerInventory::amount`
  and `PlayerSkill::level` are `unsigned int`). Library layout was already exact,
  so each was identified by its SysUtils/DB neighbours' order and size.
- **Control flow.** The guild Bank info returns inside its block and otherwise
  falls through to the next action; `Weddings::Tick` nests the countdown-7/6
  tests like the 0x12 one.
- **Unit names.** The VCL class-registration record spells the units `MainForm`
  and `MySQLthread`; the files are now `MainForm.*` and `MySQLthread.*`
  (`build.sh`/`build_asm.sh` resolve the file case from the Pascal-normalised
  export name).
- **Literal pool.** The unreferenced `""` helpers moved to where the reference's
  unreferenced NUL bytes are.

### Package init order: the `_INIT_` table (closed)

The `.data` tables at `0x55b206` (`_INIT_`) and `0x55b524` (`_EXIT_`) list every
unit's `Initialize`/`Finalize` in package-init order (6-byte entries
`{0, 0x1e, addr}`). ilink32 derives the order, and the rule is now known exactly
(a model of it reproduces our table 65/65 and every synthetic probe):

- A post-order DFS over units, roots in link order, **children visited in link
  order**.
...
- The subtlety is the EH tables. bcc32 names them with a **per-translation-unit
  counter**: `@_$DC<n>$@...`, `@_$ECT<n>$@...`, `@_$CH<n>$@...`, `<n>` little-endian
  base 26 (`A`=0; `RHB` = 17 + 7*26 + 1*676 = 875). So the `__odtbl__` of
  `AnsiString::AnsiString()` in one object is a *different symbol* from the one in
  another whenever the two TUs had emitted a different number of tables by then,
  and a unit's inlined-helper copies can make it depend on whichever unit first
  happens to share its counter. Anything that adds or removes an EH-bearing
  function -- including the unreferenced helpers this project writes, whose
  COMDATs ilink32 drops -- shifts every later name in that unit.

Two fixes followed from it. `Npc` included `Players.h` only for `RandRange`;
declaring `RandRange` directly (byte-neutral) moves `Npc` to the reference's slot.
And `Learnvalues_FileInfo` (unreferenced; it exists for its two pool literals)
must add exactly three tables -- `try { String; String; } catch (...) {}` --
because two `String` locals alone (+2) or none leave `Filecache`'s copies of the
`std::bad_alloc` / `AnsiString` tables sharing names with `Learnvalues`', which
pulls the Learn units ahead of `Filecache`. Only those two shapes-of-invisible-
code were ever constrained this way; the model predicts both.

### Operand load order: bcc32's variable ranking (closed)

The last 13 bytes were seven register swaps in `Party_ShareExp`: the reference
loads `i` before `player` at both `player->party_ids[i]` pushes, `member` before
`player` at the id/map comparisons, and `member` before `exp` at the experience
add. `bcc32 -Od` does not allocate register variables, but it still ranks them by
weighted use count, and the ranking decides which operand of an address or
comparison is loaded first: **the lighter variable is loaded first** (ties load
the base first). Swept with probes, a use weighs **4 per enclosing `for`/`while`
loop** and **half per enclosing `if`**, whole-function; **`do`/`while` and `goto`
loops add no weight**; `this` weighs its uses + 1; spelling (`a == b` vs
`b == a`, `p[i]` vs `i[p]`, `+=`) changes nothing. In our `for` loop, `member`
(15 uses) and `server` (14) outweighed `player` and `exp`; with the second loop a
`do { ... i++; } while (i < N)` -- a positive guard instead of `continue`, so the
tail is still `inc [i]; cmp [i],10; jl` -- the weights come out in the reference's
order and the function compares 352/352 with 0 mismatched.
...
- `Party_ShareExp` (`0x46690c`) — **CONVERGED**. Not a register-allocation
  floor: bcc32's use-count ranking decides the operand load order, and the second
  loop is a `do`/`while` (which adds no loop weight). See "Operand load order:
  bcc32's variable ranking".
