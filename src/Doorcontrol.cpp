...
// Provisional cross-unit views. Mapcontrol and Map are not yet reconstructed;
...
class Mapcontrol
{
  public:
    std::vector<Map> maps;
    char pad_04[0x44 - sizeof(std::vector<Map>)];
};

Doorcontrol::Doorcontrol(Mapcontrol *map_control)
{
...
void Doorcontrol::Tick(Doorcontrol *self)
{
    ...
    for (std::vector<Map>::iterator map_iter = self->map_control->maps.begin();
