    ...
    if (self->loaded == 0)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                shop.class_requirement = self->DecodeNumber(data.SubString(namelen + 6, 1));
...
void ShopValues::AddTrade(ShopValues *self, ShopValue *shop, int item_id, int buy_price,
                          int sell_price, int max_amount)
{
    shop->AddTrade(item_id, buy_price, sell_price, max_amount);
}

void ShopValues::AddCraft(ShopValues *self, ShopValue *shop, int item_id,
...
{
    shop->AddCraft(item_id, ingredient_item_id_1, ingredient_amount_1,
                   ingredient_item_id_2, ingredient_amount_2,
                   ingredient_item_id_3, ingredient_amount_3,
                   ingredient_item_id_4, ingredient_amount_4);
}
...
ShopCraftIngredient ShopValues::GetCraftIngredient1(ShopValues *self, int shop_id, int craft_id)
{
    ...
    if (shop_id > 0)
    {
        if ((int)self->record_list.size() >= shop_id)
        {
            ...
            {
                std::vector<ShopCraftVal>::iterator it = self->record_list[shop_id - 1].crafts.begin();
...
ShopCraftIngredient ShopValues::GetCraftIngredient2(ShopValues *self, int shop_id, int craft_id)
{
    ShopCraftIngredient result;
    result.item_id = -1;
    result.amount = -1;
...
ShopCraftIngredient ShopValues::GetCraftIngredient3(ShopValues *self, int shop_id, int craft_id)
{
    ShopCraftIngredient result;
    result.item_id = -1;
    result.amount = -1;
...
ShopCraftIngredient ShopValues::GetCraftIngredient4(ShopValues *self, int shop_id, int craft_id)
{
    ShopCraftIngredient result;
    result.item_id = -1;
    result.amount = -1;
...
int ShopValues::GetBuyPrice(ShopValues *self, int shop_id, int item_id, int amount)
{
    ...
    if (shop_id > 0)
    {
        if ((int)self->record_list.size() >= shop_id)
        {
            ...
            {
                std::vector<ShopItemVal>::iterator it = self->record_list[shop_id - 1].trades.begin();
        ...
        if (it->id == behavior_id)
        {
            ...
            if (it->trades.size() > 0)
                for (t = it->trades.begin(); t != it->trades.end(); t++)
                {
                    ...
                    result.Insert(EncodeNumber(self, t->buy_price, 3), result.Length() + 1);
                    result.Insert(EncodeNumber(self, t->sell_price, 3), result.Length() + 1);
                    result.Insert(EncodeNumber(self, t->max_amount, 1), result.Length() + 1);
                }
            result.Insert((char)0xff, result.Length() + 1);
            if (it->crafts.size() > 0)
                for (c = it->crafts.begin(); c != it->crafts.end(); c++)
                {
                    ...
                    result.Insert(EncodeNumber(self, c->ingredient_item_ids[0], 2), result.Length() + 1);
                    result.Insert(EncodeNumber(self, c->ingredient_amounts[0], 1), result.Length() + 1);
                    result.Insert(EncodeNumber(self, c->ingredient_item_ids[1], 2), result.Length() + 1);
                    result.Insert(EncodeNumber(self, c->ingredient_amounts[1], 1), result.Length() + 1);
                    result.Insert(EncodeNumber(self, c->ingredient_item_ids[2], 2), result.Length() + 1);
                    result.Insert(EncodeNumber(self, c->ingredient_amounts[2], 1), result.Length() + 1);
                    result.Insert(EncodeNumber(self, c->ingredient_item_ids[3], 2), result.Length() + 1);
                    result.Insert(EncodeNumber(self, c->ingredient_amounts[3], 1), result.Length() + 1);
                }
        }
        it++;
    }
